module OrdersHelper
  ...
  def order_delete_warning(order)
    ...
    outstanding_packages = order.packages.unpacked.any?{|p| p.deliveries.count.zero? || p.deliveries.any?{|d| d.pending?}} || order.packages.packed.any?{|p| p.deliveries.any?{|d| d.pending?}}
    return outstanding_packages ? deliveries_confirm : no_deliveries_confirm
  end
end
