class Distributor::SettingsController < Distributor::BaseController
  ...
  def spend_limit_confirmation
    ...
    if count > 0
      render text: "Updating the minimum balance will halt #{count} customers deliveries.  #{"They will be emailed that their account has been halted until payment is made.  " if send_halt_email && current_distributor.send_email? }Are you sure?"
    else
      render text: "safe"
    end
  end
