...
Elm.ConversionPipeline.Common.make = function (_elm) {
   ...
   if (_elm.ConversionPipeline.Common.values)
...
Elm.ConversionPipeline.Graph.make = function (_elm) {
   ...
   if (_elm.ConversionPipeline.Graph.values)
   ...
   $Text = Elm.Text.make(_elm);
   var _op = {};
   var stateColors = _U.list([A3($Color.rgb,145,191,144)
   ...
   var gridWidth = 500;
   var lineHeight = 20;
   var barTextRecord = F4(function (height,width,offset,hidden) {
      var text = _U.eq(height,
      0) ? "" : $Basics.toString($Basics.abs(offset));
      return _U.cmp(offset,0) > 0 ? {text: text
                                    ,y: height + lineHeight / 2
                                    ,x: 0} : hidden ? {text: "",y: 0,x: 0} : {text: text
                                                                             ,y: offset
                                                                             ,x: width / 2 + lineHeight};
   });
   var barRect = F3(function (bar,width,label) {
      var offset = bar.offset;
      var height = bar.height;
      var newLabel = _U.eq(height,0) ? "" : label;
      var hidden = $String.isEmpty(label);
      var barText = A4(barTextRecord,
      bar.height,
      width,
      bar.count,
      hidden);
      var date = hidden ? $Maybe.Just(A2($Graphics$Collage.rotate,
      $Basics.degrees(-90),
      A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: width / 2 + 12,_1: -40},
      $Graphics$Collage.text($Text.fromString(bar.to))))) : $Maybe.Nothing;
   ...
   var barWidth = 50;
   var legendBars = function (bars) {
      return A2($List.map,
      function (bar) {
         return A3(barRect,bar,barWidth * 2,bar.label);
      },
      bars);
   };
   var gridBars = function (allBars) {
   ...
   var legend = F2(function (bars,relativeOriginY) {
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2"
      ,_0: (0 - collageWidth) / 2 + barWidth * 2
      ,_1: relativeOriginY},
      $Graphics$Collage.group(legendBars(bars)));
   });
   var collageHeight = 600;
   var graphHeight = collageHeight - 100;
   var gridLines = F2(function (barRatio,maximumTotalHeight) {
      var lineCount = 5;
      var deltaInPx = graphHeight / lineCount;
      var scaleRatio = lineCount * deltaInPx / maximumTotalHeight;
      return $List.concat(A2($List.map,
      function (y) {
         var color = _U.eq(y,0) ? A3($Color.rgb,
         0,
         0,
         0) : A3($Color.rgb,100,100,100);
         return _U.list([A2($Graphics$Collage.moveY,
                        0 - y,
                        A2($Graphics$Collage.filled,
                        color,
                        A2($Graphics$Collage.rect,gridWidth,1)))
                        ,A2($Graphics$Collage.move,
                        {ctor: "_Tuple2",_0: gridWidth / 2 + lineHeight,_1: 0 - y},
                        $Graphics$Collage.text($Text.fromString($Basics.toString($Basics.round(y / scaleRatio)))))]);
      },
      A2($List.map,
      function (n) {
   ...
   var grid = F5(function (allBars,
   period,
   relativeOriginY,
   barRatio,
   maximumTotalHeight) {
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: barWidth * 2,_1: relativeOriginY},
      $Graphics$Collage.group($List.concat(_U.list([A2(gridLines,
                                                   barRatio,
                                                   maximumTotalHeight)
   ...
   var Bar = F6(function (a,b,c,d,e,f) {
      return {label: a
             ,color: b
             ,height: c
             ,offset: d
   ...
   var segmentToBars = function (segment) {
      ...
      var counts = A2($List.map,
      function (_) {
         return _.count;
      },
      segment.states);
      var offsets = $List.concat(_U.list([A2($List.take,1,counts)
                                         ,A3($List.scanl,
                                         F2(function (x,y) {    return x + y;}),
                                         0,
   ...
   var graph = F2(function (period,segments) {
      var allBars = A2($List.map,segmentToBars,segments);
      var positiveHeights = A2($List.map,
      function (bars) {
      ...
      allBars);
      var maximumPositiveHeight = A2($Maybe.withDefault,
      0,
      $List.maximum(positiveHeights));
      var negativeHeights = A2($List.map,
      function (bars) {
         return A2($Maybe.withDefault,
         0,
      ...
      allBars);
      var maximumNegativeHeight = $Basics.negate(A2($Maybe.withDefault,
      0,
      $List.minimum(negativeHeights)));
      var maximumTotalHeight = maximumPositiveHeight + maximumNegativeHeight;
      var relativeOriginY = maximumTotalHeight / 2 - maximumPositiveHeight + 200;
      var barRatio = graphHeight / maximumTotalHeight;
      var allBarsScaled = A2($List.map,
      function (bars) {
         return A2($List.map,
         function (bar) {
            return _U.update(bar,
            {height: bar.height * barRatio,offset: bar.offset * barRatio});
         },
         bars);
      },
      allBars);
      ...
      collageWidth,
      collageHeight,
      _U.list([A2(legend,_U.list([]),relativeOriginY)
              ,A5(grid,
              allBarsScaled,
              period,
              relativeOriginY,
   ...
   return _elm.ConversionPipeline.Graph.values = {_op: _op
                                                 ,graph: graph};
};
Elm.ConversionPipeline = Elm.ConversionPipeline || {};
Elm.ConversionPipeline.make = function (_elm) {
   "use strict";
   _elm.ConversionPipeline = _elm.ConversionPipeline || {};
   if (_elm.ConversionPipeline.values)
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var stateLabelMap = function (underscore) {
      var map = $Dict.fromList(_U.list([{ctor: "_Tuple2"
                                        ,_0: "converted"
                                        ,_1: "converted"}
   ...
   var decoder = $Json$Decode.keyValuePairs($Json$Decode.$int);
   var buildUrl = F2(function (from,to) {
      return A2($Http.url,
      "https://api.buckybox.com/v1/conversion-pipeline",
      _U.list([{ctor: "_Tuple2",_0: "from",_1: from}
              ,{ctor: "_Tuple2",_0: "to",_1: to}]));
   });
   var UpdateSegment = function (a) {
      return {ctor: "UpdateSegment",_0: a};
   };
   var UpdatePeriod = function (a) {
      return {ctor: "UpdatePeriod",_0: a};
   };
   var periodSelect = function (address) {
      ...
      _U.list([A2($Html.label,
              ...
              ,A2($Html.select,
              _U.list([$Html$Attributes.id("period")
                      ,A3($Html$Events.on,
                      "change",
              ...
              F2(function (i,s) {
                 ...
                 _U.list([$Html$Attributes.value(i)
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.div,
              ...
              ,A2($Html.div,
              _U.list([]),
              _U.list([$Html.fromElement(A2($ConversionPipeline$Graph.graph,
              model.period,
              model.segments))]))]));
   });
   var NoOp = {ctor: "NoOp"};
   ...
   var report = function (result) {
      ...
      var states = A2($List.map,
      function (_p1) {
         var _p2 = _p1;
         return A2($ConversionPipeline$Common.State,
         stateLabelMap(_p2._0),
         $Basics.toFloat(_p2._1));
      },
      cleanResult);
      var to = $Date.fromTime($Basics.toFloat($Basics.snd(A2($Maybe.withDefault,
      {ctor: "_Tuple2",_0: "",_1: 0},
      $List.head(result)))));
      var segment = A3($ConversionPipeline$Common.Segment,
      states,
      to,
      to);
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "UpdatePeriod": return _U.update(model,{period: _p3._0});
         default: var _p4 = _p3._0;
           return _U.eq($List.length(model.segments),
           segmentCount) ? _U.update(model,
           {segments: _U.list([_p4])}) : _U.update(model,
           {segments: A2($List.append,model.segments,_U.list([_p4]))});}
   });
   var initialModel = {period: "3",segments: _U.list([])};
   var model = A3($Signal.foldp,
   update,
   initialModel,
   actions.signal);
   var main = A2($Signal.map,view(actions.address),model);
   var makeHttpResquest = function (action) {
      var loadTime = $Now.loadTime;
      var dayInMs = 86400 * 1000;
      var period = A2($Maybe.withDefault,
      0,
      $Result.toMaybe($String.toFloat(A2($Maybe.withDefault,
      initialModel.period,
      $List.head(A2($List.drop,
      1,
      A2($String.split,"\"",$Basics.toString(action))))))));
      var periodInMs = 365.25 / 12 * period * dayInMs;
      var timestamps = A2($List.map,
      function (index) {
      ...
      function (timestamp) {
         var toAsInt = A2($Maybe.withDefault,
         0,
         $Result.toMaybe($String.toInt($Basics.toString(timestamp))));
         var from = $ConversionPipeline$Common.timestampToISO(timestamp - periodInMs);
         var to = $ConversionPipeline$Common.timestampToISO(timestamp);
         var list = A2($Http.get,decoder,A2(buildUrl,from,to));
         return A2($Task.andThen,
         A2($Task.map,
         function (states) {
            return A2($List._op["::"],
   ...
   var httpRequests = Elm.Native.Task.make(_elm).performSignal("httpRequests",
   function () {
      var needsFocus = function (action) {
         var _p5 = action;
         if (_p5.ctor === "UpdatePeriod") {
               return true;
            } else {
               return false;
            }
   ...
   return _elm.ConversionPipeline.values = {_op: _op
                                           ,Model: Model
