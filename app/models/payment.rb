class Payment < ActiveRecord::Base
  ...
  attr_accessible :account, :account_id, :amount, :kind, :description, :distributor, :bank_statement, :reference, :source
  ...
  def update_account
    self.transaction = account.add_to_balance(amount, kind: 'payment', description: "[ID##{id}] Recieved a payment by #{source.humanize.downcase}.")
    account.save
    save
  end
