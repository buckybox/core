class Invoice < ActiveRecord::Base
  ...
  def starting_balance
    balance - transactions.inject(EasyMoney.new(0)) { |sum, t| sum += t[:amount] }
  end
  ...
  def calculate_amount
    ...
    self.amount = deliveries.inject(EasyMoney.new(0)) { |sum, occurrence| sum += occurrence[:amount] } - balance
