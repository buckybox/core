...
describe Distributor::InvoiceInformationController do
  ...
  describe '#create' do
    context 'with valid params' do
      ...
      specify { flash[:notice].should eq("Bank Info was successfully created.") }
      specify { assigns(:invoice_information).gst_number.should eq('123-456-789') }
      specify { response.should redirect_to(bank_info_distributor_settings_url(@distributor)) }
    end
  ...
  describe '#update' do
    context 'with valid params' do
      ...
      specify { flash[:notice].should eq("Bank Info was successfully updated.") }
      specify { assigns(:invoice_information).gst_number.should eq('123-456-789') }
      specify { response.should redirect_to(bank_info_distributor_settings_path(@distributor)) }
    end
    context 'with invalid params' do
      before(:each) do
        @invoice_information = Fabricate(:invoice_information, distributor: @distributor, gst_number: '000-000-000')
