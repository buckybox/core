...
package :redis_config do
  ...
  redis_text = File.read(File.expand_path(File.join(File.dirname(__FILE__), 'configs', 'redis', 'redis.conf')))
  tmp_file = "/tmp/redis"

  push_text(redis_text, tmp_file) do
  ...
  verify do
    matches_local(redis_text, remote_file)
  end
end
